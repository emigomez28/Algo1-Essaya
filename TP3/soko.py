...
NORTE = (0, -1)
SUR = (0, 1)
LISTA_DIR = [NORTE, SUR, ESTE, OESTE]

DIRECCIONES = {'OESTE':OESTE,
                'ESTE':ESTE,
                'NORTE':NORTE,
                'SUR':SUR
}
...
def completar_desc(desc, longitud):
    nueva_desc = list()
    for elemento in desc:
        cantidad_de_paredes = longitud - len(elemento)
        if longitud > len(elemento):
            elemento += ESPACIO * cantidad_de_paredes
        nueva_desc.append(elemento)
    return nueva_desc

def obtener_niveles(ruta_entrada):
    desc = []
    niveles = dict()
    try:
        with open(ruta_entrada, 'r') as entrada:
            linea = entrada.readline().rstrip()
            max_len = 0
            for linea in entrada:
                posible_nivel = linea[:5]
                if posible_nivel == LEVEL:
                    nivel, nro = linea.rstrip().split(" ")
                    key = nivel + "_" + nro
                    continue
                if linea == "\n":
                    desc_completa = completar_desc(desc, max_len)
                    grilla = crear_grilla(desc_completa)
                    niveles[key] = grilla
                    desc.clear()
                    continue
                else:
                    if linea[:1] in CARACTERES_VALIDOS:
                        line = linea.rstrip()
                        desc.append(line)
                        if len(line) > max_len:
                            max_len = len(line)
...
def cargar_config(ruta_entrada):
    config = dict()
    try:
        with open(ruta_entrada, 'r') as entrada:
            linea = entrada.readline().rstrip()
            for linea in entrada:
                ...
                config[tecla] = accion.rstrip()
            return config
    except IOError:
        print('No se encontro el archivo')

def estado_a_string(estado): #Notar que estado es una lista de listas (matriz)
    res = ""
    for fila in estado:
        acumulador = ""
        for columna in fila:
